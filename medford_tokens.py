...
from mfdls.medford_models import Entity
...
#update to match new schema pydantic 2.+
_SCHEMA_DEFS = "$defs"
...
'''def get_available_tokens() -> Dict[str, List[str]]:
    ...
    for major in majors.values():
        #print("major")
        #print(major)
        #every "property" in the schema gets the $def dictionary
...
def _get_minor_tokens(major: Dict, definitions: Dict) -> Dict[str, List[str]]:
    ...
    #def_path = next(
    #    ref[_SCHEMA_REF]
    #    for ref in major[_SCHEMA_ITEMS]
    ...
    #},
    minors=None
    #if "anyOf" in major.keys():
    #   def_name= major["AnyOf"]["prefixItems"][_SCHEMA_REF]
    #   def_name = def_path.split(_SCHEMA_DEFS_PATH)[-1]
    #   minors = _extract_minors_from_def(definitions[def_name])
    #if "items" in major.keys():
    #   def_name= major["items"]["prefixItems"][_SCHEMA_REF]
    #   def_name = def_path.split(_SCHEMA_DEFS_PATH)[-1]
    #   minors = _extract_minors_from_def(definitions[def_name])

    #if _SCHEMA_REF in major.keys():
    #    def_name=major[_SCHEMA_REF]
    ...
    if "anyOf" in major.keys():
        ...
        for i in range(len(temp)):
            if("items" in temp[i].keys()):
                temp2=temp[i]["items"]["prefixItems"]
                for j in range(len(temp2)):
                    if _SCHEMA_REF in temp2[j].keys():
                        def_path=temp2[j][_SCHEMA_REF]
                        def_name = def_path.split(_SCHEMA_DEFS_PATH)[-1]
                        #print(def_name)
                        minors = _extract_minors_from_def(definitions[def_name])
                           
    elif("items" in temp.keys()):
        temp2=temp["items"]["prefixItems"]
        for j in range(len(temp2)):
                    if _SCHEMA_REF in temp2[j].keys():
                        def_path=temp2[j][_SCHEMA_REF]
                        def_name = def_path.split(_SCHEMA_DEFS_PATH)[-1]
    ...
    #if minors!=None and set(minors) == _DATA_PROV_NAMES:
    #    print(major)
...
def get_available_tokens() -> Dict[str, List[str]]:
    import MEDFORD as MEDFORD
    return(MEDFORD.get_major_minors())
